...
class HomepageController extends Controller
{
    ...
    public function index()
    {
        ...
        $cData->products = category::where("parent_id", 74)->get();
        $cData->referanslar = category::where("parent_id", 76)->get();
        $cData->blog = post::where("category_id", 114)->get();
        $cData->contact = contact::all()->first();
        $cData->category = category::where("parent_id", 71)->get();
        $cData->hizmetlerimiz = category::where("parent_id", 82)->get();
    ...
    public function products(Request $request)
    {
        $cData = new \ArrayObject();
        $cData->category = category::where("id", $request->category_id)->first();
        $cData->posts = category::where("parent_id", $request->category_id)->get();
    ...
    {
        $cData = new \ArrayObject();
        $cData->history = post::where("category_id", 79)->orderBy('sort')->get();
    ...
    public function urun(Request $request)
    {
        ...
        $cData->product = product::where("category_id", $request->id)->get();
        $cData->productimages = productimages::where("category_id", $request->id)->get();
    ...
    {
        $cData = new \ArrayObject();
        $cData->allProducts = category::where("parent_id", 0)->where("type", 1)->get();
        return view('home.allProductCategory', ['cData' => $cData]);
    ...
    public function referanslar(Request $request)
    {
        ...
        $cData->referanslar = category::where("parent_id", 76)->get();
        return view('home.referanslarCategory', ['cData' => $cData]);

    }

    public function referansDetay(Request $request)
    {
        $cData = new \ArrayObject();
        $cData->referanslar = post::where("category_id", request("id"))->get();
        return view('home.referanslar-detay', ['cData' => $cData]);
    }

    public function haberler(Request $request)
    {
        $cData = new \ArrayObject();
        $cData->haberler = post::where("category_id", 77)->get();
    ...
    {
        $cData = new \ArrayObject();
        $cData->posts = post::where("category_id", 82)->first();
        $cData->hizmetlerimiz = category::where("parent_id", 82)->get();
        if (request("page") == 2) return view('home.hizmetlerimiz2', ['cData' => $cData]);
        else return view('home.hizmetlerimiz', ['cData' => $cData]);
    }

    public function hizmetlerimizDetay(Request $request)
    {
        $cData = new \ArrayObject();
        $cData->hizmetlerimiz = post::where("category_id", $request->id)->where("theme", 1)->get();
        $cData->videolar = post::where("category_id", $request->id)->where("theme", 2)->get();
        $cData->back = post::where("category_id", $request->id)->where("theme", 3)->first();
        return view('home.hizmetlerimizDetay', ['cData' => $cData]);
    }


    public function kalite(Request $request)
    {
        $cData = new \ArrayObject();
        $cData->kalite = post::where("category_id", 110)->first();
        return view('home.kalite', ['cData' => $cData]);
    ...
    public function blogDetay(Request $request)
    {
        $cData = new \ArrayObject();
        $cData->data = post::find(request("id"));
        return view('home.blogDetay', ['cData' => $cData]);
    }
}
